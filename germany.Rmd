...
library(htmlwidgets, warn.conflicts = F)
...
plots <- lapply(regions, function(region) {
    fname <- paste0(
        "data/estimates/Germany_",
        if(region == "Deutschland") "" else paste0(region, "_"),
        "RKI_R.csv"
    )
    estimates <- read.csv(fname)
    estimates$date <- as.Date(estimates$date)

    plot_repronum(estimates, region, language = "de", unreliable = 2)
})

out <- lapply(seq_along(plots), function(i) {
    tab_header <- knitr::knit_expand(text = sprintf("### %s\n", regions[i])) 
    #chunk_start <- knitr::knit_expand(text = "\n```{r, echo = F, fig.width = 10, warning = FALSE, message = FALSE}") 

    plt_fname <- str_interp("plots/germany/${regions[i]}.html")
    saveWidget(
        partial_bundle(plots[[i]]),
        plt_fname,
        selfcontained = F,
        libdir = "libs"
    )
    # plot_content <- knitr::knit_expand(text = sprintf("\nplots[[%d]]", i))
    plot_content <- knitr::knit_expand(
        text = as.character(
            tags$iframe(
                src = plt_fname,
                scrolling = "no",
    ...
    fname <- if (i == 1) "Germany_RKI_R.csv" else paste0("Germany_", regions[i], "_RKI_R.csv")
    download_text <- "<details> 
    <summary> Daten herunterladen (Datenstand ${format(Sys.time(), '%d.%m.%Y, %H:%M', tz = 'Europe/Berlin')}) </summary>

    Die Daten zu dieser Grafik können Sie <a href='data/estimates/${fname}'>hier</a> herunterladen.

    Diese Datei enthält folgende Spalten:
    <ul>
        <li><b>date</b> das Datum,</li>
        <li><b>new.cases</b> die Anzahl an neuen Fällen an diesem Tag,</li>
        <li><b>repronum</b> die geschätzte Reproduktionszahl an diesem Tag,</li>
        <li><b>repronum.se</b> der (geschätzte) Standardfehler der Reproduktionszahl,</li>
        <li><b>ci.lower</b> die untere Grenze des 95% Konfidenzintervalls und</li>
        <li><b>ci.upper</b> die obere Grenze des 95% Konfidentervalls.</li>
    </ul>
    </details>
    \n"
#
