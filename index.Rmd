...
make_plot <- function(country_name) {
    ...
    ggplot(estimates, aes(x = date, y = R.H)) +
        ...
        geom_line(aes(y = RH.wday, color = "remove weekday effect"), data = ~head(., -2)) +
        geom_line(aes(y = RH.wday, color = "remove weekday effect"), data = ~tail(., 3), alpha = .2) +
        scale_color_manual(
            name = "",
            values = c("estimator" = "black", "remove weekday effect" = "blue"),
            labels = c("estimator", "remove weekday effect")
            ) +
        theme_bw() +
