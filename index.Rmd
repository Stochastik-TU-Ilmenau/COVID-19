...
make_plot <- function(country_name) {
    ...
    ggplot(estimates, aes(x = date, y = R.H)) +
        geom_line(aes(color = "estimator"), data = ~head(., -2), alpha = 1, show.legend = TRUE) +
        geom_line(aes(color = "estimator"), data = ~tail(., 3), alpha = .3, linetype = 2) +
        # bug in ggplotly, so we have to split our lines into two pieces
        # see https://github.com/tidyverse/ggplot2/issues/2006
        geom_ribbon(
            aes(color = "estimator", ymin = R.H.lower, ymax = R.H.upper), 
            alpha = 0.2,
            data = ~head(., -2)
            ) +
        geom_line(aes(y = RH.wday, color = "weekday effect removed"), data = ~head(., -2), size = 1.5) +
        geom_line(aes(y = RH.wday, color = "weekday effect removed"), data = ~tail(., 3), size = 1.5, alpha = .2) +
        scale_color_manual(
            name = "",
            values = c("estimator" = "black", "weekday effect removed" = "blue"),
            labels = c("estimator", "weekday effect removed")
            ) +
        theme_bw() +
        scale_y_log10() + # explicit ylim can lead to NaN
