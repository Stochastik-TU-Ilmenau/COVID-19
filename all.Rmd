...
# Filter countries with too few cases.
# Days from "mincases" (see plot.r) to yesterday
# needs to be "> report.delay" (see estimator.r)
df_num_days <- subset(johns_hopkins, tot.cases > mincases, select = c(reg0.name, tot.cases))
num_days <- aggregate(tot.cases ~ reg0.name, df_num_days, length)
countries <- num_days$reg0.name[num_days$tot.cases > report.delay]
...
Above, we show results for the all countries with at least `r mincases` cases `r report.delay` days ago; results for the world as well as the largest 20 countries may be found [here](index.html). In addition,  results for Germany and its Bundesländer are available [here](RKI.html) (in German).
